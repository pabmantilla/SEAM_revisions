...
## for the Dev_20 library (23 Dev sequences only, DeepSHAP attribution, hierarchical clustering)
...
# Only Dev task for Dev_20 library
tasks = ['Dev']

# Path configurations (relative to this script's directory)
MUTAGENESIS_LIBRARY_PATH = '../library_creation/Dev_20_mutagenisis_library'
DEV20_LIBRARY_PATH = '../library_creation/Dev_20_library/Dev_20/dev_20_library.pkl'
MODEL_DIR = '../data_and_models/models'
RESULTS_DIR = 'results'

## Load Dev_20 library
with open(DEV20_LIBRARY_PATH, 'rb') as f:
    libraries = pickle.load(f)
    dev_loci = libraries['dev']

print(f"Loaded Dev_20 library with {len(dev_loci)} sequences")
...
def load_library_100k(seq_idx):
    """Load the full 100K library for a Dev_20 sequence."""
    filepath = f'{MUTAGENESIS_LIBRARY_PATH}/Dev/seq_{seq_idx}/100K.h5'
...
### START SEAM - Attribution Generation

# Get all sequence indices from the Dev_20 library
...
for seq_idx in seq_indices:
    print(f"\n{'='*50}")
    print(f"Processing Dev seq_{seq_idx}")
    print(f"{'='*50}")

    # Quick check: skip entirely if ALL attribution files already exist
    if all_attributions_exist(seq_idx):
        print(f"All attribution files exist for seq_{seq_idx} - skipping")
        continue

    # Load full 100K library
    seqs_100k, preds_100k, orig_idx, library_index = load_library_100k(seq_idx)
    
    # Compute or load 100K attributions
    attr_100k_path = f'{RESULTS_DIR}/attribution_maps/deepSHAP/Dev/seq_{seq_idx}/100K.h5'
    if attributions_exist(seq_idx):
        ...
        checkpoint_path = f'{RESULTS_DIR}/attribution_maps/deepSHAP/Dev/seq_{seq_idx}/checkpoint.h5'
    ...
    for size_label, size in subset_sizes.items():
        ...
        if size_label == '100K':
            ...
            # Create subset using library_index with fixed seed
            subset_idx = create_subset_indices(library_index, size, seed=42)
        ...
        # Save attribution subset with subset_idx for verification
        save_attributions(attr_path, attrs, orig_idx, subset_idx=subset_idx)
        print(f"Saved {size_label} attributions with subset_idx")
        print(f"Seqs: {seqs.shape}, Preds: {preds.shape}, Attrs: {attrs.shape}")


### Hierarchical Clustering with 30 clusters

def cluster_and_save(attrs, seq_idx, size_label, n_clusters=30):
    """Cluster attributions and save results. Skip if already exists."""

    cluster_dir = f'{RESULTS_DIR}/cluster_metadata/Dev/seq_{seq_idx}/{size_label}'
...
def all_cluster_files_exist(seq_idx):
    """Check if all cluster files already exist for a given sequence."""
    for size_label in subset_sizes.keys():
...
for seq_idx in seq_indices:
    ...
    if all_cluster_files_exist(seq_idx):
        print(f"Skipping seq_{seq_idx} - all cluster files already exist")
        continue
    ...
    for size_label in reversed(list(subset_sizes.keys())):
        ...
        linkage, labels = cluster_and_save(attrs, seq_idx, size_label, n_clusters=cluster_number)


### MetaExplainer - Generate MSM from cluster metadata

def load_library_subset(seq_idx, size_label):
    """Load sequences and predictions for a specific subset size."""
    # Load 100K library
    seqs_100k, preds_100k, orig_idx, library_index = load_library_100k(seq_idx)
    
    if size_label == '100K':
        return seqs_100k, preds_100k, orig_idx, None
    else:
        # Create subset using same logic as attribution generation
        subset_idx = create_subset_indices(library_index, subset_sizes[size_label], seed=42)
        return seqs_100k[subset_idx], preds_100k[subset_idx], orig_idx, subset_idx


def generate_and_save_msm(seq_idx, size_label, n_clusters=30, gpu=True):
    ...
    csm_dir = f'{RESULTS_DIR}/CSM/Dev/seq_{seq_idx}/{size_label}'
    msm_path = os.path.join(csm_dir, 'msm.csv')

    # Skip if MSM already exists
    if os.path.exists(msm_path):
        print(f"Skipping seq_{seq_idx}/{size_label} - MSM already exists")
        return None

    # Check if cluster metadata exists
    if not os.path.exists(linkage_path) or not os.path.exists(labels_path):
        print(f"Skipping seq_{seq_idx}/{size_label} - cluster metadata not found")
        return None
    ...
    attr_path = f'{RESULTS_DIR}/attribution_maps/deepSHAP/Dev/seq_{seq_idx}/{size_label}.h5'
    if not os.path.exists(attr_path):
        print(f"Skipping seq_{seq_idx}/{size_label} - attributions not found")
        return None
    attributions = load_attributions(attr_path)

    # Load sequences and predictions
    seqs, preds, orig_idx, subset_idx = load_library_subset(seq_idx, size_label)
...
# Run MSM generation for all sequences and sizes
for seq_idx in seq_indices:
    print(f"\n{'='*50}")
    ...
    for size_label in reversed(list(subset_sizes.keys())):
        print(f"\n--- {size_label} ---")
        generate_and_save_msm(seq_idx, size_label, n_clusters=cluster_number, gpu=True)


### Variance Summary and Correlation Analysis

def compute_and_save_variance_summary(seq_idx, size_label):
    ...
    if os.path.exists(variance_path):
        print(f"Skipping seq_{seq_idx}/{size_label} - variance summary already exists")
        return None

    # Check if MSM exists
    if not os.path.exists(msm_path):
        print(f"Skipping seq_{seq_idx}/{size_label} - MSM not found")
        return None
...
def plot_variance_summary(seq_idx, size_label):
    ...
    if os.path.exists(plot_path):
        print(f"Skipping plot for seq_{seq_idx}/{size_label} - already exists")
        return

    # Check if variance summary exists
    if not os.path.exists(variance_path):
        print(f"Skipping plot for seq_{seq_idx}/{size_label} - variance summary not found")
        return
    ...
    plt.title(f'Dev seq_{seq_idx} - {size_label} - Entropy Variance by Position')
    plt.tight_layout()
...
def plot_combined_variance_summary(seq_idx):
    ...
    csm_base_dir = f'{RESULTS_DIR}/CSM/Dev/seq_{seq_idx}'
...
def all_variance_files_exist(seq_idx):
    """Check if all variance summary files already exist for a given sequence."""
    # Check combined plot
    csm_base_dir = f'{RESULTS_DIR}/CSM/Dev/seq_{seq_idx}'
...
# Run variance summary computation and plotting for all sequences
for seq_idx in seq_indices:
    # Skip if all variance files already exist
    if all_variance_files_exist(seq_idx):
        print(f"Skipping seq_{seq_idx} - all variance files already exist")
        continue
    ...
    for size_label in subset_sizes.keys():
        print(f"\n--- {size_label} ---")
        compute_and_save_variance_summary(seq_idx, size_label)
        plot_variance_summary(seq_idx, size_label)
...
def compute_library_size_correlations(seq_idx):
    ...
    variance_100k_path = f'{RESULTS_DIR}/CSM/Dev/seq_{seq_idx}/100K/variance_summary.csv'
    if not os.path.exists(variance_100k_path):
        print(f"Skipping seq_{seq_idx} - 100K variance summary not found")
        return None
    ...
    for size_label in size_order:
        variance_path = f'{RESULTS_DIR}/CSM/Dev/seq_{seq_idx}/{size_label}/variance_summary.csv'
        if not os.path.exists(variance_path):
            continue
    ...
    if len(results) < 2:
        print(f"Skipping seq_{seq_idx} - need at least 2 library sizes for correlation")
        return None
...
def plot_correlation_vs_library_size(seq_idx):
    ...
    # Skip if plot already exists
    if os.path.exists(plot_path):
        print(f"Skipping correlation plot for seq_{seq_idx} - already exists")
        return

    # Check if correlation file exists
    if not os.path.exists(corr_path):
        print(f"Skipping correlation plot for seq_{seq_idx} - correlation file not found")
        return
    ...
    plt.title(f'Dev seq_{seq_idx} - Correlation with 100K vs Library Size')
    plt.legend()
    plt.xscale('log')
...
def all_correlation_files_exist(seq_idx):
...
def plot_combined_correlation_all_seqs():
    """Plot correlation with 100K for all sequences on a single figure."""
    output_dir = f'{RESULTS_DIR}/hyperparam_set_correlation/Dev'
    plot_path = os.path.join(output_dir, 'correlation_vs_library_size_all_seqs.png')

    # Skip if plot already exists
    if os.path.exists(plot_path):
        print(f"Skipping combined correlation plot - already exists")
        return

    # Find all sequence folders
    csm_task_dir = f'{RESULTS_DIR}/CSM/Dev'
    if not os.path.exists(csm_task_dir):
        print(f"Skipping combined correlation plot - no CSM directory")
        return
    ...
    for seq_folder in seq_folders:
        ...
        corr_path = f'{RESULTS_DIR}/hyperparam_set_correlation/Dev/seq_{seq_idx}/correlation_with_100k.csv'
        if os.path.exists(corr_path):
            seq_data[seq_idx] = pd.read_csv(corr_path)

    if len(seq_data) == 0:
        print(f"Skipping combined correlation plot - no correlation files found")
        return
    ...
    plt.title('Dev - Correlation with 100K Reference (All Sequences)')
...
for seq_idx in seq_indices:
    ...
    if all_correlation_files_exist(seq_idx):
        print(f"Skipping seq_{seq_idx} - all correlation files already exist")
        continue
    ...
    compute_library_size_correlations(seq_idx)
